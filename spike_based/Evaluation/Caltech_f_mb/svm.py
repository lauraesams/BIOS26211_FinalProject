...
def predict_c():
    ...
    frV1_part = np.load('./output/frE2_data_set.npy')
    nbr_cells,nbr_elem,nbr_patches = (np.shape(frV1_part))

    frV1_part /=np.max(frV1_part)

    inpt = np.load('./samples/caltech_f_b.npy')
    ...
    for i in range(n_rep):
        ...
        # create a new svm
        clf = LinearSVC(C=1,multi_class='ovr',verbose=0)
...
def predict_s():
    ...
    frV1_part = np.load('./output/frE1_data_set.npy')
    nbr_cells,nbr_elem,nbr_patches = (np.shape(frV1_part))
