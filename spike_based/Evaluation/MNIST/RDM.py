...
def calcDisVal(v1,v2,formular):
    ...
    if formular == 0:
        ...
        return(np.linalg.norm(v1 - v2)/len(v1))
    elif formular == 2:
        return (1- (np.dot(v1.T,v2) / (np.linalg.norm(v1)* np.linalg.norm(v2)) ))
        #return(1 - ( np.dot(v1.T,v2) / np.sqrt( np.dot(v1.T,v1) * np.dot(v2.T,v2)) ))
    elif formular == 3:
        covM = np.cov((v1,v2), rowvar=0)
                
        iv = linalg.pinv(covM)
        mD = distance.mahalanobis(v1,v2,iv)
        print(mD)
        return(mD)
    elif formular ==4:
...
def calcRDM_imgs(test_img,l_idx):
    ...
    plt.figure()
    plt.imshow(corrM,cmap='jet', interpolation='none')#,vmin=0.0, vmax=1.2)
    plt.xlabel('class index')
    plt.ylabel('class index')
    plt.xticks(np.linspace(n_examples/2 ,n_classes*n_examples-n_examples/2-1,10), np.linspace(0,10-1,10,dtype='int16') )
    plt.yticks(np.linspace(n_examples/2 ,n_classes*n_examples-n_examples/2-1,10), np.linspace(0,10-1,10,dtype='int16') )
